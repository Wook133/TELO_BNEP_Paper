...
def train_simulator(net, settings, cycle=0):
    ...
    epochs = settings['snn_settings'].simulator_train
    if (cycle == 9):
        epochs = (int)(settings['snn_settings'].simulator_train / 5.0)
    if (cycle == 10):
        epochs = (int)(settings['snn_settings'].simulator_train / 10.0)
    elif (cycle == 11):
        epochs = (int)(settings['snn_settings'].simulator_train / 15.0)
    elif (cycle == 12):
        epochs = (int)(settings['snn_settings'].simulator_train / 20.0)
    elif (cycle == 13):
        epochs = (int)(settings['snn_settings'].simulator_train / 25.0)
    elif (cycle == 14):
        epochs = (int)(settings['snn_settings'].simulator_train / 30.0)
...
def main(env_name, shared_directory, num_processors, num_evaluations, population_size, max_num_episodes, initial_sigma,
         bne_initial_sigma, bne_weight_decay, layer_1, layer_2, use_max_action, probability_recording, dynamics_model,
    ...
    cur_settings = argument_settings(env_name=env_name, shared_directory=shared_directory,
                                     ...
                                     bne_initial_sigma=bne_initial_sigma, bne_weight_decay=bne_weight_decay,
    ...
    reset_schedule = [5000, 15000, 25000, 30000, 50000, 75000, 100000, 300000, 600000, 9000000, 11000000, 20000000, 20000000, 40000000]#, 9000000]
    ...
    for i in range(settings['snn_settings'].bne_cycles):
        ...
        if i >= 11:
            ...
            current_middle_population_size = 128
        ...
        while j < reset_schedule[schedule_counter]:
            if (j == 0) and ((i == 11) or (i == 10) or (i == 9) or (i == 4)):
                middle_cmaes = CMAES(num_params=settings['actor_settings'].num_parameters,
                                     sigma_init=settings['actor_settings'].initial_sigma,
            ...
            cur_max_timestep_outer, cur_max_reward_outer, string_out = utils.calculate_maximum_timestep_count_1(
                ...
                rate_of_change_reward=1.05, rate_of_change_timestep=1.05)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env_name', type=str, help='The name of the Environment.', default="HalfCheetahMuJoCoEnv-v0")
    ...
    parser.add_argument('--num_processors', type=int, help='Number of processors to use.', default=24)
    ...
    parser.add_argument('--initial_mu', type=float, help='Initial Mu starting point', default=0.024711240296222625)

    parser.add_argument('--initial_sigma', type=float, help='Initial Sigma starting point', default=0.1)

    parser.add_argument('--rank_fitness', type=int, help='Whether to use Rank Fitness in ES.', default=0)
    '''
    parser.add_argument('--sigma_init', type=float, help='Minimum budget used during the optimization.', default=0.25)
    '''
    ...
    parser.add_argument('--layer_2', type=int, help='Layer 1 size', default=64)

    parser.add_argument('--use_max_action', type=int, help='Whether Policy uses Max Action or Not', default=1)
    parser.add_argument('--bne_initial_sigma', type=float, help='Initial Value of Sigma for CMA-ES for BNE',
                        default=0.1)

    parser.add_argument('--bne_weight_decay', type=float, help='Initial Value of Sigma for CMA-ES for BNE',
                        default=0.001)
    ...
    parser.add_argument('--bne_cycles', type=int, help='Maximum Number of Episodes to use', default=14)
    ...
    bne_initial_sigma = args.__getattribute__("bne_initial_sigma")
    bne_weight_decay = args.__getattribute__("bne_weight_decay")
    ...
    solutions = main(env_name=env_name, shared_directory=shared_directory, num_processors=num_processors,
                     ...
                     bne_initial_sigma=bne_initial_sigma, bne_weight_decay=bne_weight_decay, initial_mu=initial_mu,
                     min_bound=min_bound, max_bound=max_bound)
