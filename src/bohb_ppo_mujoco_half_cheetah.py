...
class stablebaselinesWorkerPPO(Worker):
    ...
    def get_configspace():
        ...
        E:\Lengau\october2020\bohb\PPO\AntPyBulletEnv-v0\1606523442.5114942
5 000 000 [64, 32]
{'batch_size': 64, 'clip_range': 0.4, 'ent_coef': 2.3817923494682895e-08, 'gae_lambda': 0.99, 'gamma': 0.95, 'learning_rate': 3.376157866814762e-05, 'max_grad_norm': 0.3, 'n_epochs': 20, 'n_steps': 512, 'sde_sample_freq': 64, 'vf_coef': 0.061440886673640005}
It achieved accuracies of 975.270085 (validation) and 974.270085 (test).

        """
        cs = CS.ConfigurationSpace()
        batch_size = CSH.CategoricalHyperparameter('batch_size', choices=[16])
        n_steps = CSH.CategoricalHyperparameter('n_steps', choices=[128])
        gamma = CSH.CategoricalHyperparameter('gamma', choices=[0.9])
        learning_rate = CSH.CategoricalHyperparameter('learning_rate', choices=[3.059597163375442e-05])
        ent_coef = CSH.CategoricalHyperparameter('ent_coef', choices=[0.0002213635154208916])
        clip_range = CSH.CategoricalHyperparameter('clip_range', choices=[0.2])
        n_epochs = CSH.CategoricalHyperparameter('n_epochs', choices=[5])
        gae_lambda = CSH.CategoricalHyperparameter('gae_lambda', choices=[0.99])
        max_grad_norm = CSH.CategoricalHyperparameter('max_grad_norm', choices=[0.3])
        vf_coef = CSH.CategoricalHyperparameter('vf_coef', choices=[0.2550119529195426])
        sde_sample_freq = CSH.CategoricalHyperparameter('sde_sample_freq', choices=[8])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_budget', type=float, help='Minimum budget used during the optimization.', default=100000000)
    parser.add_argument('--max_budget', type=float, help='Maximum budget used during the optimization.', default=100500000)
    parser.add_argument('--env_name', type=str, help='Size of the Population', default="HalfCheetahMuJoCoEnv-v0")
    ...
    parser.add_argument('--n_processors', type=int, help='Number of workers to run in parallel.', default=24)
    parser.add_argument('--layer_1', type=int, help='Number of Neurons in Layer 1.', default=64)
    parser.add_argument('--layer_2', type=int, help='Number of Neurons in Layer 2.', default=64)
