...
It achieved accuracies of 975.270085 (validation) and 974.270085 (test).
        ...
        batch_size = CSH.CategoricalHyperparameter('batch_size', choices=[64])
        n_steps = CSH.CategoricalHyperparameter('n_steps', choices=[512])
        gamma = CSH.CategoricalHyperparameter('gamma', choices=[0.95])
        learning_rate = CSH.CategoricalHyperparameter('learning_rate', choices=[3.376157866814762e-05])
        ent_coef = CSH.CategoricalHyperparameter('ent_coef', choices=[2.3817923494682895e-08])
        clip_range = CSH.CategoricalHyperparameter('clip_range', choices=[0.4])
        n_epochs = CSH.CategoricalHyperparameter('n_epochs', choices=[20])
        ...
        vf_coef = CSH.CategoricalHyperparameter('vf_coef', choices=[0.061440886673640005])
        sde_sample_freq = CSH.CategoricalHyperparameter('sde_sample_freq', choices=[64])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_budget', type=float, help='Minimum budget used during the optimization.', default=60000000)
    parser.add_argument('--max_budget', type=float, help='Maximum budget used during the optimization.', default=60500000)
    parser.add_argument('--env_name', type=str, help='Size of the Population', default="AntPyBulletEnv-v0")
    ...
    parser.add_argument('--n_processors', type=int, help='Number of workers to run in parallel.', default=1)
